---
source: Local
kind: Community
start: 2010-10-28
end: 2010-10-28
url: https://wiki.eclipse.org/Eclipse_MDD_Day
organization: Eclipse Foundation
role: Co-Organizer & Speaker
skills:
  - Model-driven engineering
  - Public speaking
  - Community building
---

# Eclipse MDD Day 2010

> Co-organized Eclipse MDD Day, brought it to UBS in Zürich-Altstetten and spoke on models as the enterprise's first-class artifacts.

## Achievements

- Co-organized Eclipse MDD Day on October 28, 2010 and hosted it at UBS in Zürich-Altstetten, with itemis co-supporting — the first of the Eclipse events brought to UBS.
- Presented “User Story: Models as First Class Citizens in the Enterprise”, on a program with Ed Merks (EMF project lead), Sven Efftinge (Xtext project lead), Eike Stepper (CDO project lead) and practitioners from SAP, Robert Bosch, RWE and Nord/LB.
- Argued that requirements, analysis and design belong in formal models rather than prose, and demonstrated a proof-of-concept SDLC modeling platform on CDO and EMF.
- Named what the proof of concept had shown the Eclipse Modeling Platform still lacked: metamodel evolution that migrates existing instances, traceability across metamodels, and compare and merge for parallel work.

## References

| What | URL |
| --- | --- |
| Event write-up | https://madmeierslife.blogspot.com/2010/10/eclipse-enterprise-modeling-day-ubs.html |
| Presentation slides | https://wiki.eclipse.org/images/8/8e/MDDDay2010_UBS.pdf |
| Session abstract | https://wiki.eclipse.org/User_Story:_Models_as_First_Class_Citizens_in_the_Enterprise |
